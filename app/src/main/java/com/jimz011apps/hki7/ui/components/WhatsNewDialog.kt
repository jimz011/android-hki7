...
private val beta7Notes = listOf(
    ...
    "Fixed: in light mode with the default header, the header title/subtitle and the pull-down menu labels (Edit, Home Settings, Settings) could be near-white and unreadable. They now take their color from the surface they actually sit on, so they're readable on every theme, custom color, and wallpaper.",
    "Settings › About now has a \"What's new\" button that opens these notes any time, plus a \"Full changelog\" button that opens the complete changelog on GitHub.",
