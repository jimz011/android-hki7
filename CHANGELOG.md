...
- Buttons and header badges can now show one of an entity's attributes instead of its state — for example a sensor's battery level or a media player's current source. Open the item's settings and pick State or one of the listed attributes. (The unused per-button Label field has been removed.)
