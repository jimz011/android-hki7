...
- New "Custom popup" action for buttons, badges, and dialog buttons. Point any tap, hold, or double tap at a popup you build yourself: it opens with the usual dialog title, status, history, and close controls, and you fill it with any widget the app offers. Popups are listed under Settings › Appearance › Popups, where "Edit contents" opens one ready to arrange, and they are shared, so several buttons can open the same one. Giving a popup a status entity fills its status line and enables the history view, and stacks inside a popup can use up to six columns instead of the three a dashboard page allows.
